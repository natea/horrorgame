...
## Jane - a terrifying crawler that haunts the third floor
...
@onready var model: Node3D = $JaneModel
@onready var anim_player: AnimationPlayer = $JaneModel/AnimationPlayer
...
func _physics_process(delta: float) -> void:
	...
	# Only chase if player is on third floor (Y > 7)
...
func catch_player() -> void:
	...
	print("Jane caught you on the third floor!")

	if player.has_method("on_caught_by"):
		player.on_caught_by("jane")
